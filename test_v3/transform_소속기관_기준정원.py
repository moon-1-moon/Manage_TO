...
input_file = os.path.join(base_dir, "(251001)소속기관 기준정원(부서명 입력).xlsx")
...
def transform_data():
    ...
    try:
        ...
        # User said headers at 6, 7, 8 rows. 
        # Inspection showed strictly meaningful data at Indices 6, 7, 8 (Rows 7, 8, 9).
        # Mapping based on inspection:
        # Index 5 (Row 6): Job Group (e.g., 일반직) - Contains '일반직'
        # Index 6 (Row 7): Job Grade (e.g., 3급,4급) - Contains '3급,4급'
        # Index 7 (Row 8): Job Series (e.g., 행정) - Contains '행정' in valid cols
        
        # Columns K (Index 10) to BO (Index 66) -> Slice 10:67
        
        job_groups = df.iloc[5, 10:67].ffill().values
        job_grades = df.iloc[6, 10:67].ffill().values
        job_series = df.iloc[7, 10:67].ffill().values
        ...
        # Range(11, 599) includes 11 up to 598.
        for row_idx in range(11, 599):
            ...
            # Iterate through the counts columns K to BO (Indices 10 to 66)
            row_data = df.iloc[row_idx, 10:67].values
            ...
            for col_idx, count_val in enumerate(row_data):
                ...
                if val > 0:
                    ...
                    # Use a small epsilon for float comparison safety or just > 0 checks
                    if frac_part > 0.0001:
                        new_row_frac = [
                            dept_info[0], dept_info[1], dept_info[2], dept_info[3], dept_info[4], dept_info[5],
                            job_groups[col_idx],
        ...
        # Save to file
        output_filename = get_output_filename(base_dir, "(251001)소속기관_기준정원_데이터.xlsx")
